<h1 align="center">Hey 👋 I'm Devanshi</h1>
<h3 align="center">ML Engineer | MLOps | Data Science</h3>

🌱 Learning MLOps & Cloud Deployment  
💡 Interested in real-world ML systems  
☕ Powered by curiosity & coffee  

### 🛠 Core Skills
- ![Machine Learning](https://img.shields.io/badge/Machine%20Learning-0A66C2?style=for-the-badge)
- ![Deep Learning](https://img.shields.io/badge/Deep%20Learning-8E44AD?style=for-the-badge)
- ![Python](https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white)
- ![Pandas](https://img.shields.io/badge/Pandas-150458?logo=pandas&logoColor=white)
- ![ScikitLearn](https://img.shields.io/badge/Scikit--Learn-F7931E?logo=scikitlearn&logoColor=white)

**What I know:**
- 🧠 **Machine Learning** – Classification, Regression, Evaluation Metrics, Regularization Techniques, Hyperparameter Tuning, Feature Engineering, Data Analysis, Data Preparation
- 🤖 **Deep Learning** – ANN, CNN (theory + hands-on), Data Augumentation 
- ⚙️ **MLOps** – Experiment tracking, Model versioning, embedding model into API, Containerizing ML applications


### 📚 Currently Learning & Improving
- ![MLOps](https://img.shields.io/badge/MLOps-6A5ACD?style=for-the-badge)
- ![MLflow](https://img.shields.io/badge/MLflow-1F77B4?logo=mlflow&logoColor=white)
- ![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white)
- ![AWS](https://img.shields.io/badge/AWS-FF9900?logo=amazonaws&logoColor=white)

**Current focus:**
- ⚙️ **Advanced ML** – Learning to build end-to-end ML professional pipelines
- 📦 **MLOps** – Moving the whole MLOps pipeline to Cloud and integrating CI/CD
- 🐳 **Docker** – Learning deeper Docker concepts





### 📊 GitHub Stats
<img src="https://github-readme-stats.vercel.app/api?username=DevanshiMishra028&show_icons=true&theme=tokyonight" />

✨ *Learning every day, one bug at a time.*
